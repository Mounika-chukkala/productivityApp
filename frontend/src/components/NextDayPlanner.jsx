...
const NextDayPlanner = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = update
        ? await axios.patch(`${import.meta.env.VITE_BACKEND_URL}/update-plan`, data, {
            headers: { Authorization: `Bearer ${token}` },
          })
        : await axios.post(`${import.meta.env.VITE_BACKEND_URL}/create-plan`, data, {
            headers: { Authorization: `Bearer ${token}` },
          });
  ...
  const fetchPlan = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/get-plan?date=${new Date().getDate() + 1}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10 bg-[#f8f9f4] mt-10 border border-[#D9E3C9]">
      <h2 className="text-3xl font-bold text-[#3D4F2C] mb-8 text-center tracking-tight">Plan Your Next Day</h2>

      {/* Events */}
      <section className="mb-10">
        <h3 className="text-xl font-semibold text-[#4B5D38] mb-4">Events / Meetings</h3>
        {events.map((event, idx) => (
          <div key={idx} className="flex flex-col sm:flex-row gap-4 mb-4">
            <input
              type="time"
              value={event.time}
              onChange={(e) => handleEventChange(idx, "time", e.target.value)}
              className="border border-[#C3CFB5] px-3 py-2 text-[#2F3C1E] bg-white w-full sm:w-40 focus:outline-none"
            />
            <input
              type="text"
              value={event.title}
              onChange={(e) => handleEventChange(idx, "title", e.target.value)}
              placeholder="Event Title"
              className="flex-1 border border-[#C3CFB5] px-3 py-2 text-[#2F3C1E] bg-white focus:outline-none"
            />
          </div>
        ))}
        <button onClick={addEvent} className="text-sm text-[#6B8E23] hover:underline">
          + Add Event
        </button>
      </section>
      ...
      {/* Goals */}
      <section className="mb-10">
        <h3 className="text-xl font-semibold text-[#4B5D38] mb-4">Goals to Achieve</h3>
        {goals.map((goal, idx) => (
          <input
            key={idx}
            type="text"
            ...
            onChange={(e) => handleGoalChange(idx, e.target.value)}
            placeholder="Write a goal..."
            className="w-full border border-[#C3CFB5] px-3 py-2 mb-3 text-[#2F3C1E] bg-white focus:outline-none"
          />
        ))}
        <button onClick={addGoal} className="text-sm text-[#6B8E23] hover:underline">
          + Add Goal
        </button>
      </section>
      ...
      <div className="text-center">
        <button
          onClick={handleSubmit}
          className="bg-[#6B8E23] hover:bg-[#5F7F1D] text-white px-8 py-2 font-medium tracking-wide transition-colors duration-200"
        >
          Save Plan
        </button>
      </div>
